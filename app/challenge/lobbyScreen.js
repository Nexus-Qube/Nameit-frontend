...
import { createLobby, joinLobby, fetchLobbies } from '../../services/api';
...
export default function LobbyScreen() {
  ...
  const loadLobbies = async () => {
    setLoading(true);
    const data = await fetchLobbies(Number(topicId));
    setLobbies(data || []);
    setLoading(false);
  };
  ...
  const handleCreateConfirm = async () => {
    ...
    const lobby = await createLobby(Number(topicId), newLobbyName.trim(), player.id);
    if (!lobby) return;
  ...
  const handleJoinConfirm = async () => {
    ...
    try {
      ...
      router.push(
        `/challenge/waitingRoom?code=${modalCode.trim()}&playerId=${joinedPlayer.id}&playerName=${player.name}`
      );
    } catch (err) {
      console.error(err);
  ...
  // Open join modal for a specific lobby
  ...
  const renderLobby = ({ item }) => (
    <View style={styles.lobbyItem}>
      <Text style={styles.lobbyText}>
        Lobby {item.code} ({item.players?.length || 0} players)
      </Text>
      <TouchableOpacity
  ...
  return (
    <View style={styles.container}>
      ...
      <Text style={styles.subTitle}>Existing Lobbies:</Text>
      {loading ? (
